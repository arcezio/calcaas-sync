...
export function autostartTarget(): string | null {
...
export function enableAutostart(): string {
  if (process.platform !== 'win32') {
    throw new Error(
      `Automated autostart is currently Windows-only. On ${process.platform}, see the README ` +
  ...
  const target = autostartTarget() as string;
...
export function startWatchDetached(): void {
